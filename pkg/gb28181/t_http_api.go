...
type PlayInfo struct {
	...
	// StreamIndex 码流索引（0=主，1=子，2=第三...），用于生成 Subject/SDP 中的索引扩展（海康等常用）。
	StreamIndex int `json:"-" form:"-" url:"-"`
}
type ReqPlay struct {
	PlayInfo
}
...
// ResponseErrorWithMsg / ResponseSuccess 由 lalmax 的 gin 使用；本项目 HTTP API 在 logic 层直接返回 JSON，不依赖此二函数。
