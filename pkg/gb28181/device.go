...
func (d *Device) Catalog(conf GB28181Config) int {
	...
	// 输出Sip请求设备通道信息信令
	base.Log.Info("SIP->Catalog request:", request.String())
...
func (d *Device) CreateRequest(Method sip.RequestMethod, conf GB28181Config) (req sip.Request) {
	d.sn++
...
func (d *Device) Subscribe(conf GB28181Config) int {
...
func (d *Device) UpdateChannelStatus(deviceList []*notifyMessage, conf GB28181Config) {
	for _, v := range deviceList {
		switch v.Event {
		case "ON":
			base.Log.Debug("receive channel online notify")
			d.channelOnline(v.ChannelId)
		case "OFF":
			base.Log.Debug("receive channel offline notify")
			d.channelOffline(v.ChannelId)
		case "VLOST":
			base.Log.Debug("receive channel video lost notify")
			d.channelOffline(v.ChannelId)
		case "DEFECT":
			base.Log.Debug("receive channel video defect notify")
			d.channelOffline(v.ChannelId)
		case "ADD":
			base.Log.Debug("receive channel add notify")
			channel := ChannelInfo{
		...
		case "DEL":
			//删除
			base.Log.Debug("receive channel delete notify")
			d.deleteChannel(v.ChannelId)
		case "UPDATE":
			base.Log.Debug("receive channel update notify")
			// 更新通道
			channel := ChannelInfo{
				ChannelId:    v.ChannelId,
				ParentId:     v.ParentId,
...
func (d *Device) channelOnline(channelId string) {
	if v, ok := d.channelMap.Load(channelId); ok {
		...
		base.Log.Debug("channel online, channelId: ", channelId)
	} else {
		base.Log.Debug("update channel status failed, not found, channelId: ", channelId)
	}
}
...
func (d *Device) channelOffline(channelId string) {
	if v, ok := d.channelMap.Load(channelId); ok {
		...
		base.Log.Debug("channel offline, channelId: ", channelId)
	} else {
		base.Log.Debug("update channel status failed, not found, channelId: ", channelId)
	}
}
...
func (d *Device) UpdateChannelPosition(channelId string, gpsTime string, lng string, lat string) {
	if v, ok := d.channelMap.Load(channelId); ok {
		...
		c.Longitude = lng
		c.Latitude = lat
		base.Log.Debug("update channel position success")
	} else {
		//如果未找到通道，则更新到设备上
		d.GpsTime = time.Now() //时间取系统收到的时间，避免设备时间和格式问题
		d.Longitude = lng
		d.Latitude = lat
		base.Log.Debug("update device position success, channelId:", channelId)
	}
}
