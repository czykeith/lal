...
在原有 `gb28181` 配置基础上，新增 `upstream_config_file`（独立文件），用于集中管理“上级平台账号 + 订阅流列表”：
...
- **`subs`**：该上级平台在本节点上“订阅”的本地流列表（以 `stream_name` 为标识）。只有在此列表中的流，上级才可以点播（INVITE）。
...
5. **停止点播（上级 BYE）**
   - 上级发送 BYE；
   - 中间平台根据 `Call-ID` 清理对应的上级会话，注销 Sink，停止向该上级推送该路 RTP。
