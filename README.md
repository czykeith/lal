...
| POST | `/api/ctrl/gb28181_upstreams_conf_set` | 覆盖写入上级配置文件 `gb28181_upstreams.json`（仅写盘，不重载） |
| POST | `/api/ctrl/gb28181_upstreams_reload` | 从配置文件重载上级配置，仅刷新上级模式运行时状态 |
...
独立的上级配置文件（如 `conf/gb28181_upstreams.json`）结构如下：
...
- **覆盖写入上级配置文件**
  ...
  **行为：**
  ...
  - 仅写盘，不会重启或刷新任何运行时状态（便于先编辑配置，多次保存）。
...
- **重载上级配置并刷新运行时状态**
  ...
  **行为：**
  ...
  3. 调用 `GB28181Server.ReloadUpstreams` 仅刷新上级平台相关配置：
     - 为新配置中新增的上级创建 `upstreamServer` 并启动 `REGISTER + Keepalive` 循环；
     - 为仍存在的上级更新其运行时配置（SipID/Realm/MediaIP 等），不打断已有会话；
     - 为被删除的上级：
       - 主动向上级发送 BYE 结束所有已存在的播放会话；
       - 关闭对应的上级 Sink（停止将流转发给该上级）；
       - 停止其 `runUpstreamLoop` 循环。
  ...
  **注意：**
  ...
  - 建议通过 `..._conf_set` 写盘后，再调用 `..._reload` 让最新配置在不中断业务的情况下生效。
