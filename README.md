...
2. SDP 与 Subject 对齐主流可播放实现，以提升设备兼容性：
   ...
   - **SDP**：`o=<channelId>`、`s=Play`、`t=<start> <end>`（NTP 时间戳）、`y=<ssrc>`，无 `u=`/`f=` 字段；倍速通过 `a=scale:<value>` 携带
...
- **倍速参数**：`scale` 支持任意正数，通过 SDP 的 `a=scale` 字段实现
  - 常见值：0.25（1/4 倍速）、0.5（1/2 倍速）、1.0（正常速度）、2.0（2 倍速）、4.0（4 倍速）
  - 默认值为 1.0（正常速度）
