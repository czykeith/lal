...
> 已合并到 `GET /api/stat/gb28181_upstreams` 的返回中。

**接口地址：** `GET /api/stat/gb28181_upstreams`

**说明：**

- 返回上级平台配置列表，并附带运行时状态字段，重点是 `registered`（是否注册成功）。
- `registered` 的判断口径：最近一次 `REGISTER` 成功时间 `last_ok_at` 是否仍在该上级的 `register_validity` 有效期内。
    ...
    "upstreams": [
      {
        ...
        "registered": true,
        "last_ok_at": "2026-03-19T10:12:34.123456789+08:00",
        "retry_count": 0,
        "next_retry_at": "",
        "comment": "一级中心平台 A"
      }
...
- `registered`: 是否注册成功（在有效期内）
- `last_ok_at`: 最近一次 REGISTER 成功时间（RFC3339Nano；空表示从未成功）
- `retry_count`: 注册失败后的重试次数（运行时）
- `next_retry_at`: 下次退避重试时间（RFC3339Nano；空表示未退避/立即）
