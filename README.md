...
**注意事项：**
- 如果使用Docker部署，需要映射SIP端口和RTP端口范围：
  ```bash
  -p 5060:5060/udp -p 30000-60000:30000-60000/udp
  ```
- **设备状态监控**：服务器会自动监控设备状态，根据设备实际心跳间隔动态确定超时阈值。设备离线时会自动清理相关流信息和通道状态。
- **服务重启恢复**：服务重启后，如果收到未注册设备的心跳，会返回401要求设备重新注册，确保设备状态正确。
...
```json
{
  ...
  "stream_type": 1                         // 码流类型（0=主码流，1=辅码流，默认1）
}
```
...
```json
{
  ...
  "scale": 1.0                              // 倍速播放（1.0=正常速度，2.0=2倍速，0.5=0.5倍速，默认1.0）
}
```
...
```bash
# 使用 curl 启动GB28181回放（正常速度，使用默认流名称）
...
2. SDP消息体符合GB28181标准格式：
   - `s=Playback`：标识回放操作
   - `u=<channelId>:0`：指定回放通道和类型
   - `t=<start> <end>`：指定回放时间段（NTP时间戳格式）
   - `a=scale:<value>`：倍速参数（通过SDP的a字段携带）
3. 设备响应 200 OK，开始推送回放RTP流
4. 服务器在指定端口接收RTP流并解析为音视频数据
...
t=3874286400 3874320000
m=video 30000 RTP/AVP 96 98
a=recvonly
a=rtpmap:96 PS/90000
a=rtpmap:98 H264/90000
a=fmtp:98 profile-level-id=42E028;packetization-mode=1
a=scale:2.0
```

**注意事项：**
- **协议标准**：本接口完全符合GB28181国家标准协议，使用标准的SIP INVITE方法和SDP格式
- **时间格式**：支持 `2006-01-02T15:04:05` 或 `2006-01-02 15:04:05` 格式
- **时间范围**：`end_time` 必须晚于 `start_time`，时间在SDP中使用NTP时间戳格式（从1900-01-01开始的秒数）
- **倍速参数**：`scale` 支持任意正数，通过SDP的 `a=scale` 字段实现（GB28181标准方式）
  - 常见值：0.25（1/4倍速）、0.5（1/2倍速）、1.0（正常速度）、2.0（2倍速）、4.0（4倍速）
  - 默认值为1.0（正常速度）
- **流名称**：`stream_name` 参数可选，如果指定则使用指定的名称，否则自动生成 `device_id_channel_id_playback` 格式的名称
- **流冲突**：如果指定的 `stream_name` 已存在，接口会返回错误，避免流名称冲突
- **自动停止**：回放结束后，设备会自动发送 BYE 请求停止流
- **Subject头**：使用GB28181标准格式 `<channelId>:<streamType>,<serverId>:<streamType>`
